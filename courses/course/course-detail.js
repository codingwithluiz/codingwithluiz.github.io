// course-detail.js — CodingWithLuiz
// Renders a course from its JSON file.
// URL format: courses/course/?id=python-do-zero-a-automacao

(async function () {

  // ── BASE path helper ───────────────────────────────────────────────────────
  // From courses/course/ we need to go two levels up to reach the site root.
  const BASE = (() => {
    const path = window.location.pathname;
    if (path.includes('/courses/course/')) return '../../';
  ...
  if (!id) {
    document.body.innerHTML = '<p style="color:var(--muted);padding:6rem 2rem;text-align:center">Curso não encontrado.</p>';
    return;
  }
  ...
  try {
    ...
    document.body.innerHTML = '<p style="color:var(--muted);padding:6rem 2rem;text-align:center">Não foi possível carregar os dados do curso.</p>';
    return;
  }

  // ── <title> ────────────────────────────────────────────────────────────────
  document.title = course.title + ' | CodingWithLuiz';
  ...
  // Tech tags
  document.getElementById('cd-tech-tags').innerHTML =
    (course.technologies || []).map(t => `<span class="course-tag">${t}</span>`).join('');

  // ── What you'll learn ──────────────────────────────────────────────────────
  document.getElementById('cd-learn-list').innerHTML =
    (course.whatYouLearn || [])
      .map(i => `<li><i class="fas fa-check"></i>${i}</li>`)
      .join('');

  // ── Requirements ──────────────────────────────────────────────────────────
  document.getElementById('cd-req-list').innerHTML =
    (course.requirements || [])
      .map(r => `<li><i class="fas fa-circle" style="font-size:.35rem;vertical-align:middle"></i>${r}</li>`)
      .join('');
  ...
  document.getElementById('cd-includes').innerHTML =
    (course.includes || [])
      .map(i => `<li><i class="${i.icon}"></i>${i.text}</li>`)
      .join('');

  // Sidebar button
  const sidebarBtn = document.getElementById('cd-sidebar-btn');
  if (sidebarBtn) sidebarBtn.href = course.udemyUrl || '#';
  ...
  const reviewsEl = document.getElementById('cd-reviews');
  if (course.reviews && course.reviews.length > 0) {
    reviewsEl.innerHTML = course.reviews.map(r => `
        ...
        <div class="cd-review-header">
          ...
          <div>
            ...
            <div class="cd-reviewer-meta">
              <span style="color:var(--brand2)">${stars(r.rating)}</span>
              <span style="color:var(--muted2);font-size:.82rem;margin-left:.4rem">${r.date}</span>
            </div>
          </div>
        </div>
    ...
    reviewsEl.innerHTML = '<p class="cd-no-reviews">Sem avaliações ainda.</p>';
  }
